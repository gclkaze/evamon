package operations

import (
	"fyne.io/fyne/v2"
	"fyne.io/fyne/v2/container"
	"fyne.io/fyne/v2/dialog"
	"fyne.io/fyne/v2/layout"
	"fyne.io/fyne/v2/widget"

	"github.com/gclkaze/evamon/cmd/internal/models"
)

func ShowOperationsModal(
	parent fyne.Window,
	components []models.FilterComponent,
	initialRules []*models.TriggerRule,
	onSave func([]models.TriggerRule),
) {
	state := NewOperationsModalState(parent, components, initialRules)

	condTab := NewOperationsConditionTab(state)
	actTab := NewOperationsActionsTab(state)

	tab1 := container.NewTabItem("Conditions", condTab.Build())
	tab2 := container.NewTabItem("Actions", actTab.Build())

	tabs := container.NewAppTabs(tab1, tab2)

	var d dialog.Dialog

	saveBtn := buildSaveBtn(state, tabs, condTab, actTab, tab1, tab2, onSave, &d)
	cancelBtn := widget.NewButton("Cancel", func() { d.Hide() })

	state.OnChanged = func() { refreshSaveBtn(saveBtn, state) }

	tabs.OnSelected = func(ti *container.TabItem) {
		state.pruneAssignments()
		tab1.Content = condTab.Build()
		tab2.Content = actTab.Build()
		tabs.Refresh()
		refreshSaveBtn(saveBtn, state)
	}

	buttons := container.NewHBox(layout.NewSpacer(), cancelBtn, saveBtn)
	content := container.NewBorder(nil, buttons, nil, nil, tabs)

	d = dialog.NewCustomWithoutButtons("Operations Setup", content, parent)
	d.Resize(fyne.NewSize(900, 660))
	d.Show()
}

func buildSaveBtn(
	state *OperationsModalState,
	tabs *container.AppTabs,
	condTab *OperationsConditionTab,
	actTab *OperationsActionsTab,
	tab1 *container.TabItem,
	tab2 *container.TabItem,
	onSave func([]models.TriggerRule),
	d *dialog.Dialog,
) *widget.Button {
	btn := widget.NewButton("Save", func() {
		for _, rule := range state.SelectedRules {
			rule.BreakLink(state.AvailableComponents)
		}
		result := make([]models.TriggerRule, len(state.SelectedRules))
		for i, rule := range state.SelectedRules {
			result[i] = *rule
		}
		onSave(result)
		(*d).Hide()
	})
	btn.Importance = widget.MediumImportance
	btn.Disable()
	return btn
}

func refreshSaveBtn(saveBtn *widget.Button, state *OperationsModalState) {
	if state.Differentiator.HasChanges() {
		saveBtn.Importance = widget.SuccessImportance
		saveBtn.Enable()
	} else {
		saveBtn.Importance = widget.MediumImportance
		saveBtn.Disable()
	}
	saveBtn.Refresh()
}
